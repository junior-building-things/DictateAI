...
pub fn resolve(app: &AppHandle, state: &AppState) -> Option<ModeResolution> {
    let (
        ...
        (
            settings::get(&db, "speech_model").unwrap_or_else(|_| "deepgram-nova-3".to_string()),
            ...
            settings::get(&db, "speech_nvidia_base_url").unwrap_or_default(),
    ...
    let api_ready = match speech_model.as_str() {
        "deepgram-nova-3" => !deepgram_api_key.trim().is_empty(),
        ...
        "nvidia-parakeet-tdt-0.6b-v2" | "nvidia-canary-qwen-2.5b" => {
            !nvidia_base_url.trim().is_empty()
        }
        "alibaba-qwen3-asr-flash" => !alibaba_api_key.trim().is_empty(),
        "doubao-byteplus" => {
